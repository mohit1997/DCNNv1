...
__all__ = [
    "peakdetect", "peakdetect_fft", "peakdetect_parabola", "peakdetect_sine",
    "peakdetect_sine_locked", "peakdetect_spline", "peakdetect_zero_crossing",
    "zero_crossings", "zero_crossings_sine_fit"
]
...
def peakdetect_fft(y_axis, x_axis, pad_len=20):
    ...
    fft_padded = padd(
        list(fft_data), 2**n(len(fft_data) * pad_len) - len(fft_data))
...
def zero_crossings(y_axis,
    ...
    if diff.std() / diff.mean() > 0.1:
        ...
        raise ValueError(
            "False zero-crossings found, indicates problem {0!s} or {1!s}".
            format("with smoothing window", "unhandled problem with offset"))
