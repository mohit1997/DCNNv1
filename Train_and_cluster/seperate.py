...
train_speech = 'aplawd_train/speech'
train_peaks = 'aplawd_train/peaks'
...
for i in speech_files:
	...
	print(file)
	flag = file[-6] in train_names

	src_speech = os.path.join(in_speech, file)
	src_peaks = os.path.join(in_peaks, file)

	if flag:
		dest_speech = os.path.join(train_speech, file)
		dest_peaks = os.path.join(train_peaks, file)
		# print(dest_peaks)
