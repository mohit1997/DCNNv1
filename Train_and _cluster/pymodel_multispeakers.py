...
def custom_loader(speechfolders, peakfolders, window, stride, subwindow=40, select=None):
    ...
    for i in range(len(speechfolders)):
        speechfolder = speechfolders[i]
        peakfolder = peakfolders[i]
        ...
        speech_window_list.append(speech_windowed_data)
    ...
    speech_windowed_data = np.vstack(speech_window_list)
    peak_distance = np.concatenate(peak_distance_list)
    peak_indicator = np.concatenate(peak_indicator_list)

    print(speech_windowed_data.shape)
    print(peak_distance.shape)
    print(peak_indicator.shape)
