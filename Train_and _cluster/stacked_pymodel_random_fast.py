...
class MyNet(nn.Module):
    def __init__(self, ratio=4, dropout=0):
        super().__init__()
        ...
        self.c4 = nn.Sequential(
            nn.Conv1d(15*ratio, 30, 3, padding=4, dilation=4),
            nn.SELU())
    ...
    def forward(self, x):
        ...
        # print(x.size())
        x = x.view(x.size(0), -1, np.int32(x.size(2)/float(self.ratio)))
        # print("Mohit")
        x = self.c4(x)
        x = x.view(x.size(0), -1)
        x2 = self.regressor(x)
...
def main():
    subwindow = 40
    window = 40*4
    ratio = np.int32(window/subwindow)
    print("Ratio is ", ratio)
    train_data = create_dataloader(128, 'aplawd_female_speech', 'aplawd_female_peaks', window, 3, subwindow=subwindow, select=300)
    test_data = create_dataloader(128, 'slt_speech_raw', 'slt_peaks', 160, 20, select=160)
    model = MyNet(ratio=ratio)
    save_model = Saver('checkpoints/aplawd_female_full')
    use_cuda = True
    epochs = 16

    optimizer = optim.Adamax(model.parameters(), lr=4e-3)
